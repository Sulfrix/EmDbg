#!/usr/bin/sh
# This script only works on Linux and macOS with a modern version of Bash.
# TODO: Make a version of this script for Windows.
# This script might work under WSL, however.
# Requires the GLSL compiler and spirv-cross.
...
# Note: I couldn't figure out compiling HLSL properly so I found it better to simply use Vulkan on Windows
convert_hlsl() {
  local filename=$(basename $1 .spv)
...
convert_metal() {
  ...
  if [[ "$OSTYPE" == "darwin"* ]]; then
    # MACOS WARNING: These args might not parse correctly. 
...
convert_all() {
  convert_glsl $1
  # DirectX is not supported currently, DirectL
  #convert_hlsl $1
  convert_metal $1 $2
}
